...
void WtStockVWapExeUnit::init(ExecuteContext * ctx, const char * stdCode, WTSVariant * cfg)
{
	...
	if (_min_open_lots != 0) { 
		if (_is_KC) {
			_min_open_lots = (std::max)(_min_open_lots, _min_hands);
		}
		else {
			_min_open_lots = (std::min)(_min_open_lots, _min_hands);
		}
	}
	// 确定T0交易模式
...
void WtStockVWapExeUnit::do_calc()
{
	...
	if (_is_t0)
		vailyPos = realPos;
	double target_pos = (std::max)(realPos - vailyPos, _target_pos);
	...
	// 卖要对碎股做检查
		...
		curQty = (std::min)(vailyPos, _min_open_lots);
	}
